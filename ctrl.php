...
class ctrl
{
	...
	public function verifAction(){
        ...
        if($res[0]==0){
            ...
            $_SESSION['nom']=$info[3];
            $_SESSION['id_usr']=$info[0];
            $_SESSION['prenom']=$info[4];
            $_SESSION['fct']=$info[5];
            $_SESSION['photo']=$info[7];
            $_SESSION['classe']=$info[8];
	...
	public function absaddAction()
	{

	}

	public function noteFormAction()
	{
        $cls=$this->model->classes();
        $l=count($cls);
        require "VnoteForm.php";
	}
	...
	public function AddnoteAction()
	{
        ...
        //setlocale(LC_TIME, "fr_FR.utf8",'fra');
        $info=array($id_cls,$link /*,strftime('%A, %d/%m/%Y à %H:%M')*/);
        $this->model->Addnote($info);
	}
	public function noteShowAction(){
        $id_cls=$this->model->classe($_POST['cls']);
        $info=$this->model->EdtInfo(array($id_cls[0]));
        require 'VEdt.php';
	}
	...
	public function action()
	{
		...
		switch($action)
		{
			...
			case 'noteform' : $this->noteFormAction(); break;
